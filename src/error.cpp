...
/*!
  ...
  */
errors_t Error::CalculateErrors( int16_t *i_vals, uint16_t deltaT_us ){
  for( uint8_t i=0; i<i_chs; i++ ){
  ...
  for( uint8_t e=0; e<e_chs; e++ ){
    ...
    int32_t new_err = (int32_t)(new_errs[e] - errors.error_P[e])*deltaT_us;
    ...
    new_err = (errors.error_I[e] + errors.error_P[e])/deltaT_us;
    errors.error_I[e] = CheckRange( new_err, FEEDBACK_ERROR_BOUND );
  }
  return errors;
}

void Error::SetErrorMatrix( int8_t** em ){
  for(uint8_t i=0; i<i_chs; i++){
